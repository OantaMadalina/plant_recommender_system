...
const UpdateGameModal: React.FC<UpdateGameModalProps> = ({ show, onClose, onUpdate, game, games, teams }) => {
    ...
    const firstTeam = teams?.find(team => team.id === game.firstTeamId) ?? null;
    const secondTeam = teams?.find(team => team.id === game.secondTeamId) ?? null;

    useEffect(() => {
        setFirstTeamScore(game.firstTeamScore ?? 0);
        setSecondTeamScore(game.secondTeamScore ?? 0);
        setGameDateAndTime(game.gameDateAndTime);
    }, [game]);
    ...
    useEffect(() => {
        if (game.scorers && game.firstTeamId) {
            const filteredFirstTeamScorers = game.scorers
                .filter(scorer => scorer.teamId === game.firstTeamId)
                .map(scorer => {
                    const player = firstTeamPlayers.find(p => p.id === scorer.playerId);
                    if (player) {
                        return {
                            player: player,
                ...
                .filter(scorer => scorer !== null) as Scorer[];

            setFirstTeamScorers(filteredFirstTeamScorers)
        }

        if (game.scorers && game.firstTeamId) {
            const filteredSecondTeamScorers = game.scorers
                .filter(scorer => scorer.teamId === game.secondTeamId)
                .map(scorer => {
                    const player = secondTeamPlayers.find(p => p.id === scorer.playerId);
                    if (player) {
                        return {
                            player: player,
                            minutes: scorer.minutes,
            ...
            setSecondTeamScorers(filteredSecondTeamScorers)
        }
    ...
    const isUpdateButtonDisabled = gameDateAndTime === "";
            ...
            <div className={styles.modal}>
                ...
                <div className={styles.teamsContainer}>
                    <div className={styles.team}>
                        ...
                        <span className={styles.teamSelect}>{firstTeam?.teamName}</span>
                    ...
                    <div className={styles.team}>
                        ...
                        <span className={styles.teamSelect}>{secondTeam?.teamName}</span>
                        {!secondTeam ?
                            <img src={"/assets/no-badge.png"} alt="First Team Badge" className={styles.teamBadge} /> :
                ...
                <div className={styles.buttons}>
                    <button
                        ...
                        title={isUpdateButtonDisabled ? "We need the date set" : ""}
                    >
                        Update
                    </button>
                    <button
                        ...
                        <div className={styles.playerPhotoAndGoals}>
                            ...
                            <div className={styles.playerGoals}>
                                ...
                                <div className={styles.minutesContainer}>
                                    {selectedScorer?.minutes.map((minute: any, index: any) => (
                                        <button
                                            key={index}
                                            className={styles.minuteButton}
                                            onClick={() => removeGoal(minute, selectedPlayer?.teamId === game.firstTeamId ? setFirstTeamScorers : setSecondTeamScorers)}
                                        >
                                ...
                                <AddGoalForm onAddGoal={addGoal} setFunction={ selectedPlayer?.teamId === game.firstTeamId ? setFirstTeamScorers : setSecondTeamScorers}/>
